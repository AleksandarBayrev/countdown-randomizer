  ...
  render() {
    return (
      <div className='countdown-randomizer-container'>
        ...
        <div className='buttons-wrapper'><button disabled={this.state.shouldRandomize || this.state.values.length < 2} onClick={() => this.randomizeValues()}>Randomize values</button><button disabled={this.state.shouldRandomize || this.state.randomizedValue.length === 0} onClick={() => this.resetRandomziedValue()}>Reset Randomized Value</button><button disabled={this.state.shouldRandomize || this.state.inputFieldValue.length === 0} onClick={() => { this.resetInput(); this.resetRandomziedValue(); }}>Reset input</button></div>
